...
| 2 | **Embed blocks produce empty containers** — `core/html` wrapping for `embed` and `iconify` roles generates empty `gb-element` divs. The `html` field on IRNode isn't picked up by `ir-planner.ts` → `core/html` block. | Complex sections (code-diff, forms) lost entirely | Extend `ir-planner.ts` to handle the `html` field on IR nodes by creating a `core/html` block with `innerHTML` |
