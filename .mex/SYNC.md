...
> **npm flow (this project):** the .mex/sync.sh wrapper is absent (git-clone install only). Run `npx mex-agent sync` for the interactive drift check + targeted/full resync, or `npx mex-agent check` for a one-off drift report.
