...
> **npm flow (this project):** mex was installed via `npm i -D mex-agent`, so the .mex/setup.sh wrapper is absent. Run `npx mex-agent setup` instead — it does the same thing (scaffold, tool config, scan, populate prompt). The script below is for the git-clone install.
...
- **npx mex-agent sync** — interactive drift check + targeted or full resync (npm flow; .mex/sync.sh is git-clone-only)
